...
// Helper function to calculate string length (since bpf_strlen doesn't exist)
static __always_inline int bpf_str_len(const char *str) {
    int len = 0;
    #pragma unroll
    for (int i = 0; i < 32; i++) {   // Max path length limit
        if (str[i] == '\0')
            break;
        len++;
    }
...
static __always_inline bool should_exclude_path(const char *pathname) {
    ...
    // Common exclusions - check directory prefixes
    if (bpf_strncmp(pathname, 5, "/tmp/") == 0)
        return true;
    if (bpf_strncmp(pathname, 9, "/var/log/") == 0)
        return true;
    if (bpf_strncmp(pathname, 6, "/proc/") == 0)
        return true;
    if (bpf_strncmp(pathname, 5, "/sys/") == 0)
        return true;
    if (bpf_strncmp(pathname, 5, "/dev/") == 0)
        return true;
    if (bpf_strncmp(pathname, 5, "/run/") == 0)
        return true;
...
int trace_enter_openat(struct syscall_trace_enter *ctx)
{
    ...
    if (filename) {
        ...
        if (should_exclude_path(temp_info.pathname)) {  // Use temp_info.pathname instead of filename
            return 0;  // Skip event entirely - never reaches ring buffer
        }
    }
...
int trace_close_enter(struct trace_event_raw_sys_enter *ctx)
{
    ...
    if (info) {
      // Rate limiting check
      if (should_exclude_path(info->pathname)) {
          return 0;  // Skip event entirely - never reaches ring buffer
      }
...
{   char pathname[256];
    ...
    if (!fname)
        return 0;
    // Copy the filename to local buffer
    bpf_probe_read_kernel_str(pathname, sizeof(pathname), fname); 
    
    if (should_exclude_path(pathname)) {  // Use pathname instead of fname
        return 0;
    } 
    // Send delete event
